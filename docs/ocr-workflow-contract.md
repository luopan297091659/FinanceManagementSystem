...
文件先通过浏览器的 `tasks/upload` 请求保存到系统服务器，再由系统服务器调用 Make。因此浏览器开发者工具中的 `tasks/:taskId/start` 只显示任务摘要，不会显示发往 Make 的二进制 multipart 内容。Make 返回 `502`、`503` 或 `504` 时系统会最多重试 3 次。
...
新工作流应使用 `sessionId` 作为关联键。为避免历史场景中断，系统回调端仍可兼容接收旧的 `seesinId` 或 `taskId`，并兼容 `results`、`data.records`、`data.results` 数组。失败时使用：
