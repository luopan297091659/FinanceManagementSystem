...
export class EmailSettingsService {
  ...
  private normalize(raw: unknown): EmailSettings {
    ...
    return {
      smtp_host: String(value.smtp_host || defaults.smtp_host),
      smtp_port: Number(value.smtp_port || defaults.smtp_port),
      smtp_user: String(value.smtp_user || defaults.smtp_user),
      smtp_pass: String(value.smtp_pass || defaults.smtp_pass),
      smtp_secure: value.smtp_secure ?? defaults.smtp_secure,
      from_name: String(value.from_name || defaults.from_name),
  ...
  async saveSettings(payload: Partial<EmailSettings>) {
    ...
    if (!next.smtp_host) throw new BadRequestException('SMTP host is required');
    if (!next.smtp_port || next.smtp_port < 1) throw new BadRequestException('SMTP port is invalid');
  ...
  async sendTestEmail(to: string) {
    if (!to) throw new BadRequestException('测试邮箱不能为空');
    const { settings, transporter } = await this.createTransporter();
    await transporter.sendMail({
      from: `"${settings.from_name}" <${settings.smtp_user}>`,
      to,
