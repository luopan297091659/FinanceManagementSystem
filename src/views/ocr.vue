  ...
  <section class="ocr-page">
    ...
    <template v-else>
      ...
      <div class="panel-card match-workflow-bar">
        ...
        <div class="match-history-actions"><span>匹配历史 {{ matchHistory.length }} 次</span><button class="primary-button sync-button" type="button" :disabled="syncingMatched || !syncableMatchedCount" @click="syncMatchedToDatabase">{{ syncingMatched ? '同步中…' : `同步已匹配项到数据库（${syncableMatchedCount} 条）` }}</button><button class="secondary-button" type="button" :disabled="!unresolvedCount" @click="downloadRemainingJson">导出剩余 JSON</button></div>
      </div>
...
async function deleteSelectedMatchTemplate() { const template=matchRuleTemplates.value.find((item)=>item.id===selectedMatchTemplateId.value); if (!template || !window.confirm(`确认删除规则“${template.name}”？`)) return; try { await api.deleteOcrMatchRuleTemplate(template.id); matchRuleTemplates.value=matchRuleTemplates.value.filter((item)=>item.id!==template.id); selectedMatchTemplateId.value=''; matchTemplateName.value=''; showMessage('已删除保存的匹配规则。'); } catch(error) { showMessage(error.message,true); } }
