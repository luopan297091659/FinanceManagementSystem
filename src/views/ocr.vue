...
function normalizeMatchConfiguration(configuration) { if (configuration?.rules) return cloneConfiguration(configuration); const legacy={partyName:['contract.payerName','counterparty'],summary:['contract.bankStatementSummary','contentSummary'],propertyName:['property.name','propertyName'],roomNumber:['room.roomNumber','roomNumber'],contractNumber:['contract.contractNumber','contractNumber'],amount:['contract.monthlyRent','statisticalAmount'],date:['contract.validDate','date']}; return { logicalOperator:'AND', rules:(configuration?.fields || []).map((field,index)=>({id:`legacy-${index}`,systemField:legacy[field]?.[0] || '',sourceField:legacy[field]?.[1] || '',operator:'equals',required:true})).filter((rule)=>rule.systemField) }; }
