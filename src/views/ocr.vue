<template>
  <section class="ocr-page" :class="`ocr-page--${pageMode}`">
    ...
    <template v-else-if="pageMode === 'execute'">
      <div class="page-heading">
        <div>
          ...
          <p class="muted">本次执行会自动关联本次任务，并将所有文件一次性提交到该工作流。</p>
        </div>
    ...
    <template v-else>
      <div class="page-heading">
        <div><p class="eyebrow">OCR 对账任务详情</p><h2>{{ detailTask?.taskName || '加载任务中…' }}</h2><p v-if="detailTask?.workflow?.name" class="muted">{{ detailTask.workflow.name }}</p></div>
      ...
      <div class="detail-layout">
        <div class="panel-card result-panel">
          ...
          <DualScrollTable wrapper-class="detail-table-scroll"><table class="result-table detail-result-table"><thead><tr><th>序号</th><th>原始文件</th><th>单据类型</th><th>单据日期</th><th>实际月份</th><th>摘要 / 对方</th><th>收支方向</th><th>金额</th><th>系统侧数据</th><th>匹配度</th><th>匹配状态</th><th>匹配说明</th><th class="operation-col">操作</th></tr></thead><tbody>
    ...
    <div v-if="workflowEditorOpen" class="modal-backdrop" @click.self="workflowEditorOpen = false">
      <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="workflow-dialog-title">
        ...
        <p class="contract-note">Webhook 将接收任务信息、回调地址，以及重复的 requestFile 文件字段。</p>
