  ...
  <section class="ocr-page">
    ...
    <template v-else>
      ...
      <div class="panel-card match-workflow-bar">
        <div class="workflow-actions"><span class="step-button completed-step">① 回调数据已填表（{{ detailSummary.total }} 条）</span><i>→</i><button class="primary-button step-button" type="button" @click="openMatchConfig">② 选择匹配字段 <small v-if="matchingFields.length">{{ matchingFields.length }} 项</small></button><i>→</i><button class="primary-button step-button" type="button" :disabled="matchingRunning || !matchingFields.length || !unresolvedCount" @click="runOcrMatching()">{{ matchingRunning ? '匹配中…' : `③ 执行匹配（剩余 ${unresolvedCount} 条）` }}</button></div>
      ...
      <div class="detail-layout">
        ...
        <div class="panel-card result-panel">
          ...
          <div class="table-scroll"><table class="result-table detail-result-table"><thead><tr><th>序号</th><th>原始文件</th><th>日期</th><th>摘要 / 对方</th><th>收支方向</th><th>金额</th><th>系统侧数据</th><th>匹配状态</th><th>匹配说明</th><th class="operation-col">操作</th></tr></thead><tbody><tr v-for="(record, index) in visibleRecords" :key="record._recordId || index"><td>{{ valueOf(record, 'record_no') || index + 1 }}</td><td>{{ valueOf(record, 'original_file_name', 'source_file_name', 'fileName') || detailTask?.fileNames?.[0] || '—' }}</td><td>{{ valueOf(record, 'date', 'target_month', 'transactionDate') || '—' }}</td><td>{{ valueOf(record, 'summary', 'inflow_party', 'outflow_party', 'tenant_name', 'tenantName') || '—' }}</td><td>{{ valueOf(record, 'money_direction', 'transaction_type') || '—' }}</td><td>{{ money(valueOf(record, 'net_amount', 'document_amount', 'amount')) }}</td><td><div class="system-data-cell"><strong>{{ record.systemMatch?.propertyName || '—' }} {{ record.systemMatch?.roomNumber || '' }}</strong><small>{{ record.systemMatch?.tenantName || record.systemMatch?.bankStatementSummary || record.systemMatch?.contractNumber || '—' }}</small></div></td><td><span class="status-tag" :class="matchStatusTone(record)">{{ matchStatusText(record) }}</span></td><td>{{ record.systemMatch?.reason || '—' }}</td><td class="operation-col"><div class="row-actions"><button v-if="!['MATCHED','MANUAL_SYNC'].includes(record.systemMatch?.status) && matchingFields.length" type="button" @click="runOcrMatching([record._recordId])">单独再匹配</button><button type="button" class="execute-link" @click="openMatchDialog(record)">{{ record.systemMatch?.status === 'MATCHED' ? '重新匹配' : '手工匹配' }}</button><button v-if="record.systemMatch?.status !== 'MANUAL_SYNC'" type="button" class="manual-sync-link" @click="markManualSync(record)">手工同步</button></div></td></tr><tr v-if="!visibleRecords.length"><td colspan="10" class="empty-cell">暂无符合条件的数据。</td></tr></tbody></table></div>
        </div>
      </div>
    ...
    <div v-if="matchConfigOpen" class="modal-backdrop" @click.self="matchConfigOpen = false">
      <div class="modal-card match-config-dialog" role="dialog" aria-modal="true" aria-labelledby="match-config-title">
        <div class="section-head"><div><p class="eyebrow">OCR 匹配条件</p><h3 id="match-config-title">选择匹配字段</h3></div><button class="icon-button" type="button" @click="matchConfigOpen = false">×</button></div>
        <p class="muted">所选条件按 AND 组合；记录缺少任一已选字段时会保留为待人工确认。</p>
        <div class="match-field-grid"><label v-for="field in availableMatchFields" :key="field.key" class="match-field-option"><input v-model="draftMatchingFields" type="checkbox" :value="field.key" /><span><strong>{{ field.label }}</strong><small>{{ field.help }}</small></span></label></div>
        <p v-if="!draftMatchingFields.length" class="configuration-warning">执行匹配前，请至少选择一个匹配字段。</p>
        <div class="modal-actions"><button class="ghost-button" type="button" @click="matchConfigOpen = false">取消</button><button class="primary-button" type="button" :disabled="savingMatchConfig || !draftMatchingFields.length" @click="saveMatchConfiguration">{{ savingMatchConfig ? '保存中…' : '保存匹配字段' }}</button></div>
      </div>
    </div>
...
const matchConfigOpen = ref(false), matchingFields = ref([]), draftMatchingFields = ref([]);
const savingMatchConfig = ref(false), matchingRunning = ref(false);
...
async function loadDetailTask() { try { detailTask.value = await api.getOcrTask(detailTaskId); matchingFields.value = [...(detailTask.value?.resultJson?.matchConfig?.fields || [])]; } catch (error) { showMessage(error.message, true); } }
function openMatchConfig() { draftMatchingFields.value = [...matchingFields.value]; matchConfigOpen.value = true; }
async function saveMatchConfiguration() { if (!draftMatchingFields.value.length) return; savingMatchConfig.value = true; try { detailTask.value = await api.saveOcrMatchConfig(detailTaskId, draftMatchingFields.value); matchingFields.value = [...draftMatchingFields.value]; matchConfigOpen.value = false; showMessage('匹配字段已保存，可以执行自动匹配。'); } catch (error) { showMessage(error.message, true); } finally { savingMatchConfig.value = false; } }
async function runOcrMatching(recordIds) { if (!matchingFields.value.length || matchingRunning.value) return; matchingRunning.value = true; try { detailTask.value = await api.executeOcrMatching(detailTaskId, matchingFields.value, recordIds); const summary=detailTask.value?.resultJson?.summary; showMessage(`本次匹配完成，累计已匹配 ${summary?.matched || 0} 条，剩余 ${summary?.unmatched || 0} 条。`); } catch (error) { showMessage(error.message, true); } finally { matchingRunning.value = false; } }
function downloadRemainingJson() { const payload={ taskId:detailTask.value?.taskId, sessionId:detailTask.value?.sessionId, matchingFields:matchingFields.value, records:unresolvedRecords.value, matchHistory:matchHistory.value }; const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}); const url=URL.createObjectURL(blob); const link=document.createElement('a'); link.href=url; link.download=`${detailTask.value?.taskName || 'ocr'}-remaining.json`; link.click(); URL.revokeObjectURL(url); }
