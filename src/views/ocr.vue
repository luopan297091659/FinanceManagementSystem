  ...
  <section class="ocr-page">
    ...
    <template v-else>
      ...
      <div class="panel-card match-workflow-bar">
        <div class="workflow-actions"><span class="step-button completed-step">① 回调数据已填表（{{ detailSummary.total }} 条）</span><i>→</i><button class="primary-button step-button" type="button" @click="openMatchConfig">② 配置匹配规则 <small v-if="validMatchRuleCount">{{ validMatchRuleCount }} 条</small></button><i>→</i><button class="primary-button step-button" type="button" :disabled="matchingRunning || !validMatchRuleCount || !rematchableCount" @click="runOcrMatching()">{{ matchingRunning ? '正在用最新算法重新计算…' : `③ 重新执行匹配（${rematchableCount} 条）` }}</button></div>
      ...
      <div class="metrics-grid"><div><small>合计</small><strong>{{ detailSummary.total }}</strong></div><div><small>自动匹配</small><strong>{{ detailSummary.autoMatched || 0 }}</strong></div><div><small>手工匹配</small><strong>{{ detailSummary.manualMatched || 0 }}</strong></div><div><small>手工同步</small><strong>{{ detailSummary.manualSync || 0 }}</strong></div><div><small>不适用（支出）</small><strong>{{ detailSummary.notApplicable || 0 }}</strong></div><div><small>待人工确认</small><strong>{{ detailSummary.unmatched }}</strong></div><div><small>文件数量</small><strong>{{ detailTask?.fileNames?.length || 0 }}</strong></div></div>
    ...
    <div v-if="matchConfigOpen" class="modal-backdrop" @click.self="matchConfigOpen = false">
      <div class="modal-card match-config-dialog" role="dialog" aria-modal="true" aria-labelledby="match-config-title">
        ...
        <div class="modal-actions"><button class="ghost-button" type="button" @click="matchConfigOpen = false">取消</button><button class="primary-button" type="button" :disabled="savingMatchConfig || !hasValidDraftMatchRule" @click="saveMatchConfiguration">{{ savingMatchConfig ? '保存中…' : '确认配置' }}</button></div>
      </div>
    </div>
    ...
    <div v-if="reviewingRecord" class="modal-backdrop" @click.self="closeMatchDialog">
      <div class="modal-card match-dialog" role="dialog" aria-modal="true" aria-labelledby="match-dialog-title">
        <div class="section-head"><div><p class="eyebrow">手工匹配系统数据</p><h3 id="match-dialog-title">核对银行流水与系统合同</h3><p class="muted">自动匹配要求金额严格一致、契约者假名达到配置阈值，并且候选唯一或领先至少 8 分。</p></div><button class="icon-button" type="button" @click="closeMatchDialog">×</button></div>
        ...
        <div class="candidate-list">
          <label v-for="candidate in matchCandidates" :key="candidate.id" class="candidate-row" :class="{ selected: selectedContractId === candidate.id }"><input v-model="selectedContractId" type="radio" :value="candidate.id" /><span class="candidate-main"><strong>{{ candidate.propertyName }} · {{ candidate.roomNumber || '无房间号' }}</strong><small>{{ candidate.contractNumber || '无合同号' }} · {{ candidate.tenantName || candidate.contractorName || candidate.payerName || '无租客信息' }}</small><span class="candidate-summary"><small>系统银行摘要</small><b>{{ candidate.bankStatementSummary || '无银行摘要' }}</b></span></span><span class="candidate-score"><small>匹配度</small><strong class="match-score" :class="matchScoreTone(candidate.matchScore)">{{ matchScoreText(candidate.matchScore) }}</strong><small>{{ candidateMatchFieldLabel(candidate.matchedField) }}</small></span></label>
...
const editingWorkflowId = ref(''), workflowPendingDelete = ref(null), taskPendingDelete = ref(null), taskName = ref(''), fileInput = ref(null), message = ref(''), messageIsError = ref(false);
...
const rematchableRecords = computed(() => detailRecords.value.filter((record) => record.systemMatch?.status !== 'MANUAL_SYNC' && record.systemMatch?.matchMode !== 'MANUAL'));
...
function openMatchConfig() { draftMatchConfiguration.value = normalizeMatchConfiguration(matchConfiguration.value); systemFieldSearch.value=''; sourceFieldSearch.value=''; if (!draftMatchConfiguration.value.rules.length) addMatchRule(); matchConfigOpen.value = true; }
...
async function openMatchDialog(record) { reviewingRecord.value = record; selectedContractId.value = record.systemMatch?.contractId || ''; candidateSearch.value = String(valueOf(record, 'contentSummary', 'counterpartyRaw', 'counterparty', 'contractorName', 'roomNumber', 'inflow_party', 'outflow_party', 'tenant_name', 'summary', 'room_number') || ''); matchCandidates.value = []; await loadMatchCandidates(); }
...
function matchStatusText(record) { const status = record.systemMatch?.status; if (status === 'MATCHED') return record.systemMatch?.matchMode === 'MANUAL' ? '手工匹配' : '已匹配'; if (status === 'MANUAL_SYNC') return '手工同步'; if (status === 'NOT_APPLICABLE') return '不适用'; if (status === 'AMBIGUOUS') return '多个候选'; return '待确认'; }
