  ...
  <section class="ocr-page">
    ...
    <div v-if="matchConfigOpen" class="modal-backdrop" @click.self="matchConfigOpen = false">
      <div class="modal-card match-config-dialog" role="dialog" aria-modal="true" aria-labelledby="match-config-title">
        ...
        <div class="mapping-layout">
          ...
          <main class="mapping-workspace"><div class="mapping-toolbar"><div><h4>匹配规则工作区</h4><small>规则内多选字段按“任一命中”；规则之间按各自 AND / OR 连接。</small></div><button class="secondary-button" type="button" @click="addMatchRule">＋ 添加规则</button></div>
            <div v-for="(rule, index) in draftMatchConfiguration.rules" :key="rule.id" class="mapping-rule-wrap">
              ...
              <div class="mapping-rule"><b>{{ index + 1 }}</b>
                ...
                <select v-model="rule.operator"><option v-for="operator in matchOperators" :key="operator.key" :value="operator.key">{{ operator.label }}</option></select>
    ...
    <div v-if="reviewingRecord" class="modal-backdrop" @click.self="closeMatchDialog">
      <div class="modal-card match-dialog" role="dialog" aria-modal="true" aria-labelledby="match-dialog-title">
        <div class="section-head"><div><p class="eyebrow">手工匹配系统数据</p><h3 id="match-dialog-title">核对银行流水与系统合同</h3><p class="muted">只有唯一候选达到 100% 才会自动匹配；其他结果必须由人工确认。</p></div><button class="icon-button" type="button" @click="closeMatchDialog">×</button></div>
...
const systemMatchFields = [
  ...
  { key:'contract.monthlyRent', label:'月租金额', group:'契约' }, { key:'contract.managementFee', label:'管理费', group:'契约' },
...
function normalizeMatchConfiguration(configuration) {
  ...
  return { version:2, rules:rawRules.map((rule,index)=>({
    ...
    operator:rule.operator || 'equals', required:rule.required !== false,
...
function addMatchRule() { draftMatchConfiguration.value.rules.push({ id:`rule-${Date.now()}-${Math.random().toString(36).slice(2,7)}`, systemFields:[], sourceFields:[], operator:'equals', required:true, logicalOperator:draftMatchConfiguration.value.rules.length ? 'AND' : 'AND' }); }
