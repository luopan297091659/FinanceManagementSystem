...
async function openMatchDialog(record) { reviewingRecord.value = record; selectedContractId.value = record.systemMatch?.contractId || ''; candidateSearch.value = String(valueOf(record, 'counterparty', 'contractorName', 'contentSummary', 'roomNumber', 'inflow_party', 'outflow_party', 'tenant_name', 'summary', 'room_number') || ''); matchCandidates.value = []; await loadMatchCandidates(); }
